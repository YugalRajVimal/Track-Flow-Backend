...
async function addSubTask(taskId, subTask) {
  ...
  const sanitizedSubTask = {
    ...
    printType: subTask.printType,
    date: subTask.date,
    locationStatus: subTask.locationStatus || 'warehouse', // NEW
  };
  ...
  if (sanitizedSubTask.locationStatus && !allowedStatuses.includes(sanitizedSubTask.locationStatus)) {
  ...
  // Only one subTask per task can be marked Missing
  if (sanitizedSubTask.locationStatus === 'missing') {
    const hasOtherMissing = (taskRecord.subTask || []).some(st => st.locationStatus === 'missing');
    if (hasOtherMissing) {
      throw new Error('Only one sub-task per task can be marked "Missing".');
    }
  }
...
async function editSubTask(taskId, subTaskIndex, updateData) {
  ...
  // Validate locationStatus if present
  if (Object.prototype.hasOwnProperty.call(updateData, 'locationStatus')) {
    const allowedStatuses = ['warehouse', 'missing'];
    if (!allowedStatuses.includes(updateData.locationStatus)) {
      throw new Error("Invalid 'locationStatus'. Must be 'warehouse' or 'missing'.");
    }
    if (updateData.locationStatus === 'missing') {
      const hasOtherMissing = taskRecord.subTask.some(
        (st, i) => i !== subTaskIndex && st.locationStatus === 'missing'
      );
      if (hasOtherMissing) {
        throw new Error('Only one sub-task per task can be marked "Missing".');
      }
  ...
  let newMTR = Object.prototype.hasOwnProperty.call(updateData, 'mtr')
    ? (Number(updateData.mtr) || 0)
    : (Number(taskRecord.subTask[subTaskIndex].mtr) || 0);

  let sumMTR = 0;
  for (let i = 0; i < taskRecord.subTask.length; i++) {
    sumMTR += i === subTaskIndex ? newMTR : (Number(taskRecord.subTask[i].mtr) || 0);
  }
  if (sumMTR > totalMTR) {
    throw new Error('Cannot update subTask: the sum of MTRs would exceed TotalMTR of the task');
  }
  ...
  const allowedFields = [
    'program', 'mtr', 'challanNo', 'silicateOrQuiringName',
    'sinkage', 'challanPhoto', 'printType', 'date',
    'locationStatus', // NEW
  ];
